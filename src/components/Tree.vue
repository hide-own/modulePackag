<template>
  <Tree
    ...
    :data="val"
    value-key="id"
    @change="treeChange(val, $event)"
  >
    <template #title>
      <span>{{ val.name }}</span>
    </template>
    <template #children="{ children }">
      <span class="pl-4">子元素-{{ children.name }}</span>
    </template>
    <template #menu="{ data }">
      {{ data }}
      <div v-if="data.pid === 0" class="py-2 hover:bg-gray-200" @click="addSort(data.id)">
        添加分类
      </div>
      <div class="py-2 hover:bg-gray-200" @click="modifySort(data.id)">编辑分类</div>
      <div class="py-2 hover:bg-gray-200" @click="delSort(data.id)">删除分类</div>
    </template>
  </Tree>
  ...
  <pre>
    ...
    #title                        标题行内容
    #children                     子元素标题行内容 默认name | (#children="{children}")
    #menu                         自定义菜单内容 (#menu="{data}") data 当前数据
    #icon                         图标插槽 (#icon='{open}') open:boolean 是否展开行

    @change                       {value:根据valueKey来,open:打开状态}
  </pre>
</template>
...
//分类选择的id
function treeChange(val: goods, id: number) {
  setTimeout(() => {
    val.children = [{ id: 11, pid: 0, name: '西瓜', enname: '', title: '装修图片', children: [] }]
  }, 1000)
  console.log(id)
}

const category = reactive<goods[]>([
  {
    id: 1,
    pid: 0,
    name: '装修图片',
    ...
    title: '装修图片',
    children: [
      { id: 11, pid: 0, name: '西瓜', enname: '', title: '装修图片', children: [] },
      { id: 12, pid: 0, name: '苹果', enname: '', title: '装修图片', children: [] }
    ],
    loading: false
  },
