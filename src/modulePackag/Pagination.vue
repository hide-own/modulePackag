    ...
    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
      <div>
        <p class="text-sm text-gray-700">
          共<span class="font-medium">{{ total }}</span
          >条 - {{ pageSize }}页/条
        </p>
      </div>
      <div>
        ...
        >
          ...
          <div
            :class="[
              'relative inline-flex px-4 py-2 border  text-sm font-medium items-cente',
              current === 1
                ? 'z-10 bg-indigo-50 border-indigo-500 text-indigo-600 r'
...
//目录数
const article = ref<number>(Math.ceil(total.value / pageSize.value))

//分页折叠
const fold = ref<number[]>([])
...
function info() {
  ...
  after.value =
    current.value < article.value && fold.value[fold.value.length - 1] + 1 < article.value
}
...
function changePage(val: number) {
  ...
  if (
    val >= fold.value[2] &&
    fold.value[fold.value.length - 1] + 1 < article.value &&
    val + 1 > 5
  ) {
