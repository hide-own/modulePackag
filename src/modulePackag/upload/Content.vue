  ...
  <div class="flex flex-wrap">
    <div class="w-full md:w-1/4 px-3 border-r-2 border-gray-200">
      ...
      <div class="overflow-auto h-[50vh] scrollbar">
        ...
        <!--    明细分类    -->
        <Tree menu @change="treeChange" v-for="(val,index) in searchValue??category" :key="index+'-'+val.id"
              :data="val">
          <template #title>
            <span>{{ val.name }}</span>
          </template>
          <template v-slot:menu="data">
            <div class="py-2 hover:bg-gray-200" @click="addSort(val.id)" v-if="data.data.pid===0">添加分类</div>
    ...
    <div class="w-full md:w-3/4 px-3">
      <ImageGroup :sortId="sortId" @useImg="$emit('useImg', $event)"/>
    </div>
  </div>
</template>
  ...
  [
    {
      ...
      "children": [{"id": 1, "pid": 0, "name": "西瓜", "enname": "", "title": "装修图片", "children": []},
...
let sortId = ref<number | string | null>(null)
