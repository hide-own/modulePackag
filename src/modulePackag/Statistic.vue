...
let old = ref<number>(props.value)

const value = computed(() => {
  return props.value
})
let num: number, time: number, lang: (NodeJS.Timer | null) = null
...
watch(
  ...
  () => {
    ...
    let oldInt = Math.trunc(old.value)
    let point = String(value.value).split('.')[1]
