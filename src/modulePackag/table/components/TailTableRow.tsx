    ...
    return (): VNodeChild => {
      return [
        value((): VNodeChild => {
          ...
          return (
            <tr
              key={`row${rowValue.value}`}
              class={{ 'hover:bg-gray-100': features.value.hover }}
              {...attrs}
            >
              {context.build((column, index) => {
                ...
                return (
                  <TailTableCell
                    key={`cell${column.field}${rowValue.value}`}
        ...
        value((): VNodeChild => {
          ...
          if (!ext.config?.tree) {
            ...
            return (
              <tr key={`row${rowValue.value}expanded`} class={{ hidden: !expanded }}>
                <td colspan={childrenCount}>{slot({ row, rowIndex })}</td>
              </tr>
            )
          }
