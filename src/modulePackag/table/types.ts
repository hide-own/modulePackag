...
export interface UserTableExtension<Config = unknown> {
  /** 扩展名 */
  name: string
  /** 列标题 */
...
export interface TableExtension<Config = unknown> extends UserTableExtension<Config> {
...
export type TableLoader = (
  page: number,
  perPage: number,
  cancel: UserPaginationCancelToken
) => Promise<void>

export type BuiltinExtension<Config extends Record<string, unknown>> = Config &
  Omit<UserTableExtension<Config>, 'name'>
